 ...
 * @param data Pointer to the data that should be copied to the buffer after creation (optional, if not set, no data is
 * copied over)
 *
...
/**
 * Create a buffer on the device with staging buffer
 *
 * @param usageFlags Usage flag bit mask for the buffer (i.e. index, vertex, uniform buffer)
 * @param memoryPropertyFlags Memory properties for this buffer (i.e. device local, host visible, coherent)
 ...
 * @return:
 */
void VulkanDevice::createBufferWithStagigingBuffer(VkQueue transferQueue, VkBufferUsageFlags usageFlags,
                                                   ...
                                                   VkDeviceSize size, void *data)
{
    ...
    createBuffer(VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, &stagingBuffer, size,
                 data);
    createBuffer(usageFlags, memoryPropertyFlags, buffer, size);
 ...
 * @param (Optional) begin If true, recording on the new command buffer will be started (vkBeginCommandBuffer) (Defaults
 * to false)
 *
