/**
 * Set 0: contains descriptors of the pipeline's own data
 * 2 uniform buffers (matrices and params)
 * 1 storage buffer (genrerate ray pipeline)
 * or 3 storage buffer (hit result buffer, vertex buffer and index buffer)
 * ***/
...
layout(set = 0, binding = 2) buffer readonly HitInforBuffer
{
    HitInfor hitBuffer[];
};
struct Vertex {
...
#ifdef GENERATERAY_PIPELINE
struct Ray {
    vec3 origin; /* *< The ray origin, i.e., xyz. */
    float tmin;  /* *< The min distance of potential ray hit. */
    vec3 dir;    /* *< The ray direction in the same space of AS. */
    float tmax;  /* *< The max distance of potential ray hit. */
};

layout(set = 0, binding = 2) buffer writeonly RayBuffer
{
    Ray generatedRays[];
};
#endif
...
#endif // end of not GENERATERAY_PIPELINE
